			...
			enum snd_soc_bias_level level)
{
	...
	//slove voice delay by zhouxinghua 20210605
	/*switch (level) {
	case SND_SOC_BIAS_ON:
		...
		snd_soc_write(codec, ES8311_CLK_MANAGER_REG01, 0xbF);
...
static int es8311_set_pll(struct snd_soc_dai *codec_dai, int pll_id,
...
static int es8311_probe(struct snd_soc_codec *codec)
{
	...
	printk("Enter into %s()\n", __func__);
	//ret = snd_soc_codec_set_cache_io(codec, 8, 8, 1);
	//if (ret < 0)
	//	return ret;
	...
	msleep(100);
	//es8311_set_bias_level(codec, SND_SOC_BIAS_STANDBY); //slove voice delay by zhouxinghua 20210605
	return ret;
}
