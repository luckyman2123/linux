...
//<!---[BUGID]21675 modify codec for SLM750T/V lihongbin@20181206
//#include "../codecs/rt5616.h"
#include "../codecs/nau8810.h"
//--->end
#include "../codecs/es8311.h" //add codec es8311 by zhouxinghua 20210527
/* Spk control */
#define MDM_SPK_ON 1

//#define NAU8810_CARD //Ä¬ÈÏ±àÒëes8311 ´ò¿ª±àÒënau8810 add by zhouxinghua 20210601
...
static int mdm_auxpcm_startup(struct snd_pcm_substream *substream)
{
	...
	if (atomic_inc_return(&aux_ref_count) == 1) {
		...
		if (lpass_gpio_mux_spkr_ctl_virt_addr != NULL) {
			...
			pr_err("%s lpass_gpio_mux_spkr_ctl_virt_addr is NULL\n",
				__func__);
			ret = -EINVAL;
			goto done;
		}
...
//<!---[BUGID]21675 modify codec for SLM750T/V lihongbin@20181206
/*
 *@func: setup audio hw params
 *@substream: audio private data 
 *@params: hw params, such as sample rate
 */
static int mdm_sec_auxpcm_be_hw_params(struct snd_pcm_substream *substream,
			     struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
//			| SND_SOC_DAIFMT_CBS_CFS;
    fmt =   SND_SOC_DAIFMT_DSP_A | SND_SOC_DAIFMT_CBS_CFS; 
...
//	ret = snd_soc_dai_set_pll(codec_dai, 0, RT5616_PLL1_S_MCLK,
//				  19200000, params_rate(params) * 512);
	ret = snd_soc_dai_set_pll(codec_dai, 0, 0,
				  12288000, params_rate(params) * 512);  // 2048000 as bclk, 8000 as rlclk
...
static int mdm_auxpcm_be_hw_params(struct snd_pcm_substream *substream,
			     struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_dai *codec_dai = rtd->codec_dai;
...
//del I2S DAL by caogang
	...
	/* Backend DAI Links */	
	{
		...
		.codec_name = /*"tomtom_codec",*/"msm-stub-codec.1",
		.codec_dai_name = /*"tomtom_i2s_rx1",*/"msm-stub-rx",
		//.codec_name = "nau8810.4-001a",
	...
	},
	{
		...
		.cpu_dai_name = "msm-dai-q6-mi2s.0",
		.platform_name = "msm-pcm-routing",
		.codec_name = "msm-stub-codec.1",/*"tomtom_codec",*/
		.codec_dai_name = "msm-stub-tx",/*"tomtom_i2s_tx1",*/
		//.codec_name = "nau8810.4-001a",
		//.codec_dai_name = "nau8810-hifi",
		.no_pcm = 1,
		.dpcm_capture = 1,
	...
	},
	{
		...
		//.codec_name = "msm-stub-codec.1",
		//.codec_dai_name = "msm-stub-rx",
		/* modify codec es8311 by zhouxinghua 20210527*/
		.codec_name = "es8311.4-0018",
...
static int audio_pa_gpio = 0;
static int audio_pa_mode = 0;
//read
static ssize_t show_driver_attr_audio_pa_mode(struct device_driver *ddri, char *buf)
{
	return snprintf(buf, 20, "audio_pa_mode:%d\n", audio_pa_mode);
}
//write
static ssize_t store_driver_attr_audio_pa_mode(struct device_driver *ddri, const char *buf, size_t count)
{
	pr_info("operate audio_pa_mode : %c\n",buf[0]);
	/*
	gpio_direction_output
	gpio_set_value
	*/
	gpio_set_value(audio_pa_gpio, 0);
	switch(buf[0])
	{
	case '0':
		gpio_set_value(audio_pa_gpio, 0);
		...
		break;
	case '1':
		gpio_set_value(audio_pa_gpio, 1);
		audio_pa_mode = 1;
		break;
	case '2':
		gpio_set_value(audio_pa_gpio, 1);
...
static DRIVER_ATTR(audio_pa_mode,	S_IWUSR | S_IRUGO, show_driver_attr_audio_pa_mode, store_driver_attr_audio_pa_mode);
/////////////////////////////////////////////////////
...
static int mdm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	/* enable nau8810 add by zhouxinghua 20210601*/
	unsigned char codec_name[] = "nau8810.4-001a";
	unsigned char codec_dai_name[] = "nau8810-hifi";
	//add end
...
//////////////////////////////audio pa mode//////////////////////////////////////////
	audio_pa_gpio = of_get_gpio_flags(node, 0, &flags);
	if (audio_pa_gpio < 0) { 
        	pr_info("Failed to get gpio flags, error: %d\n",audio_pa_gpio);
   	}
	ret = gpio_request(audio_pa_gpio, "audio_pa_gpio");
	if (ret) {
		pr_info("%s: unable to request gpio %d\n", __func__, audio_pa_gpio);
	}
	ret = gpio_direction_output(audio_pa_gpio, 0);
	if (ret) {
		pr_info("gpio_direction_output fail %s\n", "audio_pa_gpio");
	}

	if(driver_create_file(&mdm_asoc_machine_driver.driver, &driver_attr_audio_pa_mode))
	{
		printk("driver_attr_audio_pa_mode  driver_create_file ERROR \n");
	}
///////////////////////////////////////////////////////////////////////////////////
	...
	/* enable nau8810 add by zhouxinghua 20210601*/
	#ifdef NAU8810_CARD
		memcpy(card->dai_link[29].codec_name,codec_name,sizeof(codec_name));
		memcpy(card->dai_link[29].codec_dai_name,codec_dai_name,sizeof(codec_dai_name));
		memcpy(card->dai_link[30].codec_name,codec_name,sizeof(codec_name));
		memcpy(card->dai_link[30].codec_dai_name,codec_dai_name,sizeof(codec_dai_name));
	#endif
	//add end
