/*
  *  hs0038 driver for fs210
  *
  *  Copyright (c) 2011 http://dev.hqyj.com/
  *
  *
...
/*fs210_hs0038_keycode必须包含所有要用到的系统键值*/
static const unsigned int fs210_hs0038_keycode[]={
...
static const unsigned int fs210_hs0038_keycode_807F[]={
	0,/*NULL*/0,/*NULL*//*0~1*/
...
static const unsigned int fs210_hs0038_keycode_00FF[32]={
...
static const unsigned int fs210_keycode_00FF[32]={
	0x10,0x03,0x01,0x06,
	0x09,0x1d,0x1f,0x0d,
	0x19,0x1b,0x11,0x15,
	0x17,0x12,0x16,0x4d,
...
struct fs210_hs0038
{
	unsigned int keycode[ARRAY_SIZE(fs210_hs0038_keycode)];
...
{
	int i;
	for(i=0 ; i<sizeof(fs210_keycode_00FF)/sizeof(fs210_keycode_00FF[0]) ; i++)
	{
		if(code == fs210_keycode_00FF[i])
		{
			if(sizeof(fs210_hs0038_keycode_00FF)/sizeof(fs210_hs0038_keycode_00FF[0]) >= i)
			return  fs210_hs0038_keycode_00FF[i];
		}
	}
	return 0xFFFF;
}
...
static int fs210_hs0038_read(struct fs210_hs0038 *hs0038_data)
{
...
/*	for(i=0;i<sizeof(values)/sizeof(values[0]);i++)
...
#ifdef CONFIG_FS210_DEBUG_HS0038
		printk("\n i =%d\n", i);			
	...
	{
		i=values[2];
		input_report_key(hs0038_data->input,fs210_hs0038_keycode_807F[i], 1);
		input_report_key(hs0038_data->input,fs210_hs0038_keycode_807F[i], 0);
		input_sync(hs0038_data->input);
...
static irqreturn_t fs210_hs0038_interrupt(int irq, void *dev_id)
{
	struct fs210_hs0038 *hs0038 = dev_id;
	unsigned long irqflags = IRQF_TRIGGER_FALLING;

	disable_irq_nosync(irq);
	fs210_hs0038_read(hs0038);
...
{
	struct fs210_hs0038 *hs0038 = platform_get_drvdata(pdev);
	...
	error = request_irq(irq, fs210_hs0038_interrupt, irqflags, desc, hs0038);
	if (error) {
...
static int __devinit fs210_hs0038_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct gpio_keys_platform_data *pdata = pdev->dev.platform_data;
	struct device *dev = &pdev->dev;
	struct input_dev *input;
	struct fs210_hs0038 *hs0038;
	int i=0;

	hs0038= kzalloc(sizeof(struct fs210_hs0038),GFP_KERNEL);
	...
	input->name = "fs210_hs0038";
	input->phys = "fs210_hs0038/input1";
	...
	input->keycodemax = ARRAY_SIZE(fs210_hs0038_keycode);
	set_bit(EV_KEY, input->evbit);


	for (i = 0; i < ARRAY_SIZE(fs210_hs0038_keycode); i++){
		hs0038->keycode[i]= fs210_hs0038_keycode[i];
	...
	if (ret < 0){
		printk(KERN_ERR "fs210_hs0038.c: input_register_device() return %d!\n", ret);
		goto  err_input_free;
	}
...
static int __devexit fs210_hs0038_remove(struct platform_device *pdev)
{
	struct fs210_hs0038 *s3c_hs0038 = platform_get_drvdata(pdev);
	struct gpio_keys_platform_data *pdata = pdev->dev.platform_data;
...
#ifdef CONFIG_PM
static int fs210_hs0038_suspend(struct device *dev)
{
...
static int fs210_hs0038_resume(struct device *dev)
{
...
static const struct dev_pm_ops fs210_hs0038_pm_ops = {
	.suspend	= fs210_hs0038_suspend,
	.resume		= fs210_hs0038_resume,
};
#else
#define  fs210_hs0038_suspend NULL
#define  fs210_hs0038_resume NULL
#endif
static struct platform_driver s3c_hs0038_driver = {
	.probe		= fs210_hs0038_probe,
	.remove		= __devexit_p(fs210_hs0038_remove),
	.driver		= {
		.name   = "hs0038",
		.owner	= THIS_MODULE,
#ifdef CONFIG_PM
		.pm	= &fs210_hs0038_pm_ops,
#endif
	},
};


static int __init fs210_hs0038_init(void)
{
	return platform_driver_register(&s3c_hs0038_driver);
}

static void __exit fs210_hs0038_exit(void)
{
	platform_driver_unregister(&s3c_hs0038_driver);
}

module_init(fs210_hs0038_init);
module_exit(fs210_hs0038_exit);

MODULE_AUTHOR("http://dev.hqyj.com/");
MODULE_DESCRIPTION("fs210 GPIO Keyboard Driver");
MODULE_LICENSE("GPL");
