...
{
	unsigned char result,send_byte[2],offset,temp;
	...
	*rece_len = rece_bitlen/8;
	if((result == OK)&&(*rece_len == rece_buff[0]))
	{
		...
		#if CPU_Card_Debug
		pr_info("nfc_fm17750<- T0 = 0x%02X\n",CPU_CARD.ATS.T0);
		pr_info("nfc_fm17750<- CPU_CARD.FSD = 0x%02X\n",CPU_CARD.FSD);
		#endif
		...
		if(CPU_CARD.ATS.T0&0x40)
		{
			...
			#if CPU_Card_Debug
			pr_info("nfc_fm17750<- TA1 = 0x%02X\n",CPU_CARD.ATS.TA1);
			#endif
			offset++;
		}
		else
		{
			CPU_CARD.ATS.TA1 = 0;
			#if CPU_Card_Debug
			pr_info("nfc_fm17750<- TA1 is not transmitted\r\n");
			#endif
		}
		if(CPU_CARD.ATS.T0&0x20)
		{
			...
			#if CPU_Card_Debug
			pr_info("nfc_fm17750<- TB1 = 0x%02X\n",CPU_CARD.ATS.TB1);
			pr_info("nfc_fm17750<- CPU_CARD.ATS.FWI = 0x%02X\n",CPU_CARD.ATS.FWI);
			temp = CPU_CARD.FWT>>8;
			pr_info("nfc_fm17750<- CPU_CARD.FWT H = 0x%02X\n",temp);
			temp = CPU_CARD.FWT&0xFF;
			pr_info("nfc_fm17750<- CPU_CARD.FWT L = 0x%02X\n",temp);
			#endif
		}
		else
		{
			CPU_CARD.ATS.TB1 = 0;
			#if CPU_Card_Debug
			pr_info("nfc_fm17750<- TB1 is not transmitted\r\n");
			#endif
		}
		if(CPU_CARD.ATS.T0&0x10)
		{
			...
			#if CPU_Card_Debug
			pr_info("nfc_fm17750<- TC1 = 0x%02X\n",CPU_CARD.ATS.TC1);
			#endif
			offset++;
		}
		else
		{
			CPU_CARD.ATS.TC1 = 0;
			#if CPU_Card_Debug
			pr_info("nfc_fm17750<- TC1 is not transmitted\r\n");
			#endif
		}
	}
	else
...
{
	unsigned char result,send_byte[3];
	unsigned int rece_bitlen;
	Pcd_SetTimer(CPU_CARD.FWT);
...
/****************************************************************/
 ...
 {
		unsigned char result,i,tpdu_tx_len;
		unsigned int rece_bitlen;
...
CPU_I_BLOCK_START:
   		...
   		if (result == OK)
		{
			...
			if((CPU_CARD.PCB&0xF0) == 0xF0)//WTX
				...
				#if CPU_Card_Debug
		   			pr_info("nfc_fm17750<- I BLOCK CPU_CARD.PCB = 0x%02X\n",CPU_CARD.PCB);
		   		#endif
			*rece_len = rece_bitlen/8;
			return OK;
		}
			...
			for (i=0;i<3;i++)
				{
				   ...
				   if (result == OK)
			 		{
						#if CPU_Card_Debug
		   					pr_info("nfc_fm17750-> I Block NAK OK\r\n");//
		   				#endif
					CPU_CARD.PCB = *rece_buff;
					...
					if((CPU_CARD.PCB&0xF0) == 0xA0)//收到ACK
						{
							#if CPU_Card_Debug
		   						pr_info("nfc_fm17750-> I Block ACK\r\n");//
								pr_info("nfc_fm17750<- CPU_CARD.PCB = 0x%02X \n",CPU_CARD.PC);
		   					#endif
				...
				#if CPU_Card_Debug
		   			pr_info("nfc_fm17750-> I Block NAK ERROR\n");//
		   		#endif
		}
		*rece_len=0;
		return ERROR;
...
/****************************************************************/
 ...
 {
 	unsigned char send_buff[2],result,i;
	unsigned int rece_bitlen;
	...
	if (result == OK)
		{
			...
			if((CPU_CARD.PCB&0xF0) == 0xA0)//收到ACK
						{
							#if CPU_Card_Debug
		   						pr_info("nfc_fm17750-> R Block ACK\n");//
								pr_info("nfc_fm17750<- CPU_CARD.PCB = 0x%02X\n",CPU_CARD.PCB);
		   					#endif
						CPU_CARD.PCB=CPU_CARD.PCB&0x0F;
						CPU_PCB_CONVER();
			...
			for (i = 0 ;i<3;i++)
				{
				   ...
				   if (result == OK)
			 		{
						#if CPU_Card_Debug
		   					pr_info("nfc_fm17750-> R Block NAK OK\n");//
		   				#endif
					CPU_CARD.PCB=*rece_buff;
					if((CPU_CARD.PCB&0xF0) == 0xF0)//WTX
						{
				...
				#if CPU_Card_Debug
		   			pr_info("nfc_fm17750-> R Block NAK ERROR\n");//
		   		#endif
...
/****************************************************************/
 ...
 {
   unsigned char send_buff[3],result;
   unsigned int rece_bitlen;
   Pcd_SetTimer(CPU_CARD.FWT);
...
{
	unsigned char result,send_buff[2],rece_buff[2];
	unsigned int rece_bitlen;
	Pcd_SetTimer(CPU_CARD.FWT);
