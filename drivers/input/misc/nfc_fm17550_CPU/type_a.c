...
unsigned char TypeA_Halt(void)
{
	...
	{
		#if Type_A_Debug
		pr_info("nfc_fm17750<- result = %0x02X\n",result);
		len=rece_bitlen>>8;
		pr_info("nfc_fm17750<- rece_bitlen H = %0x02X\n",len);
		len=rece_bitlen&0xFF;
		pr_info("nfc_fm17750<- rece_bitlen L = %0x02X\n",len);
		#endif
		return ERROR;
	}
}
...
void FM175XX_Initial_ReaderA(void)
{
	...
	Write_Reg(ModWidthReg,MODWIDTH);				//MODWIDTH 106kbps
	Write_Reg(GsNOnReg,(GSNON<<4)|MODGSNON);	
	Write_Reg(CWGsPReg,GSP);
	Write_Reg(ControlReg,0x10);		//Initiator = 1
	Write_Reg(RFCfgReg,RXGAIN<<4);
	Write_Reg(RxThresholdReg,COLLLEVEL|(MINLEVEL<<4));
	ModifyReg(TxAutoReg,BIT6,SET);//Force 100ASK = 1
