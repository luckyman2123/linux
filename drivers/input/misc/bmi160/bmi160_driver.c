...
static int bmi_accel_input_init(struct bmi_client_data *client_data)
{
...
/* 	#if defined(CONFIG_USE_QUALCOMM_HAL)
	...
	dev_notice(client_data->dev,
		"bmi160 input_accel register successfully!\n");
	return err;
}
...
static int bmi_gyro_input_init(struct bmi_client_data *client_data)
{
	...
	if (NULL == gyro_dev)
		return -ENOMEM;
	gyro_dev->name = "bmi160_gyro";
	gyro_dev->id.bustype = BUS_I2C;
	input_set_capability(gyro_dev, EV_MSC, INPUT_EVENT_SGM);
	input_set_capability(gyro_dev, EV_MSC, INPUT_EVENT_STEP_DETECTOR);
	input_set_capability(gyro_dev, EV_MSC, INPUT_EVENT_FAST_GYRO_CALIB_DONE);
...
/* input_set_capability(gyro_dev, EV_ABS, ABS_MISC);
	input_set_abs_params(gyro_dev, ABS_RX,
	BMI160_GYRO_MIN_VALUE, BMI160_GYRO_MAX_VALUE,
	0, 0);
	input_set_abs_params(gyro_dev, ABS_RY,
	BMI160_GYRO_MIN_VALUE, BMI160_GYRO_MAX_VALUE,
	0, 0);
	input_set_abs_params(gyro_dev, ABS_RZ,
	BMI160_GYRO_MIN_VALUE, BMI160_GYRO_MAX_VALUE,
	0, 0); 
*/
	input_set_drvdata(gyro_dev, client_data);
	...
	client_data->gyro_input = gyro_dev;
	dev_notice(client_data->dev,
...
static void bmi_gyro_input_destroy(struct bmi_client_data *client_data)
{
	struct input_dev *gyro_dev = client_data->gyro_input;
...
static void bmi_work_func(struct work_struct *work)
{
	...
	if (err < 0)
		return;
//HAL no need the input event for SLT181
#if 0
...
static void bmi160_gyro_work_func(struct work_struct *gyro_work)
{
	struct bmi_client_data *client_data =
		container_of((struct delayed_work *)gyro_work,
			struct bmi_client_data, gyro_work);
	unsigned long delay =
		msecs_to_jiffies(atomic_read(&client_data->delay));
	struct bmi160_gyro_t gdata;
	int err;

	err = BMI_CALL_API(read_gyro_xyz)(&gdata);
	if (err < 0)
		return;

	/*report current frame via input event*/
	input_event(client_data->gyro_input, EV_REL, REL_RX, gdata.x);
	input_event(client_data->gyro_input, EV_REL, REL_RY, gdata.y);
	input_event(client_data->gyro_input, EV_REL, REL_RZ, gdata.z);
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, 16, "%d\n", atomic_read(&client_data->accl_wkqueue_en));

}
static ssize_t bmi160_gyro_enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *gyro_input = to_input_dev(dev);
	struct bmi_client_data *client_data = input_get_drvdata(gyro_input);
		...
		const char *buf, size_t count)
{
	...
	int err;
	unsigned long enable;
	int pre_enable = atomic_read(&client_data->accl_wkqueue_en);
	...
	if (enable) {
		if (pre_enable == 0) {
			...
			msecs_to_jiffies(atomic_read(&client_data->delay)));
			atomic_set(&client_data->accl_wkqueue_en, 1);
		}
		...
		if (pre_enable == 1) {
			...
			atomic_set(&client_data->accl_wkqueue_en, 0);
		}
	}
...
static DEVICE_ATTR(bmi_value, 0664,
...
static void bmi_signification_motion_interrupt_handle(
		struct bmi_client_data *client_data)
{
	printk(KERN_INFO "bmi motion\n");
	wake_lock(&client_data->wake_lock);
...
/*input_report_rel(client_data->input,INPUT_EVENT_SGM,1);*/
	...
	wake_unlock(&client_data->wake_lock);
...
static void bmi160_accel_work_fn(struct work_struct *work)
{
	...
	input_report_abs(sensor->input, ABS_Z,
		(data.z));
	/* input_event(sensor->input, EV_SYN, SYN_TIME_SEC,
...
int bmi_probe(struct bmi_client_data *client_data, struct device *dev)
{
	...
	/* input device init */
	err = bmi_accel_input_init(client_data);
	if (err < 0)
		goto exit_err_clean;
	...
	if (NULL == dev->platform_data) {
		printk(KERN_ERR "dev->platform_data = NULL\n");
	}
	if (NULL != dev->platform_data) {
	...
	if(NULL == client_data->bst_pd) {
		printk(KERN_ERR "client_data->bst_pd = NULL\n");
	}
	if (NULL != client_data->bst_pd) {
			memcpy(client_data->bst_pd, dev->platform_data,
					sizeof(*client_data->bst_pd));
	...
	INIT_DELAYED_WORK(&client_data->gyro_work, bmi160_gyro_work_func);
	...
	if (err) {
		...
		/*modify by lch for DUGID25291 20190506 start*/
		/*enable_irq_wake(&client_data->IRQ);*/
		enable_irq_wake(client_data->IRQ);
		/*modify by lch for DUGID25291 20190506 end*/
#endif
	wake_lock_init(&client_data->wake_lock, WAKE_LOCK_SUSPEND, "bmi160_sensor");
	client_data->selftest = 0;
...
exit_err_sysfs:
	if (err)
		...
		bmi_gyro_input_destroy(client_data);
...
int bmi_remove(struct device *dev)
{
	...
	if (NULL != client_data) {
		...
		bmi_input_destroy(client_data);
		bmi_gyro_input_destroy(client_data);
		wake_lock_destroy(&client_data->wake_lock);
		if (NULL != client_data->bst_pd) {
			kfree(client_data->bst_pd);
			client_data->bst_pd = NULL;
		}
...
static int bmi_post_resume(struct bmi_client_data *client_data)
{
	...
	if (atomic_read(&client_data->accl_wkqueue_en) == 1) {
