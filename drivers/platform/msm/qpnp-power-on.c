...
/* maxiaoping  20190927 bugid27493 modification for NV verification,start */
#include <soc/qcom/smem.h>
#include <linux/io.h>
#include <linux/reboot.h>
/* maxiaoping  20190927 bugid27493 modification for NV verification,end */
...
/* maxiaoping  20190927 bugid27493 modification for NV verification,start */
/* Shared structure for  Apps<->MDM data feature.*/
#define SMEM_FUNC_DELAY_TIME 30000 /*30s*/

extern void do_msm_restart(enum reboot_mode reboot_mode, const char *cmd);

struct vendor_smem_type {
	unsigned int apps_cmd;
	unsigned int apps_data;
	unsigned int mdm_cmd;
	unsigned int mdm_data;
};
struct vendor_smem_type *smem_data_ptr;

static struct delayed_work smem_func_delay_work;
/* maxiaoping  20190927 bugid27493 modification for NV verification,end */
...
/* maxiaoping  20190927 bugid27493 modification for NV verification,start */
static int apps_smem_vendor_init(void)
{
	unsigned int smem_size=0;

	pr_err("%s: Enter\n",__func__);

	smem_data_ptr = smem_get_entry(SMEM_ID_VENDOR0, &smem_size, SMEM_APPS,
				 SMEM_ANY_HOST_FLAG);
	if (IS_ERR_OR_NULL(smem_data_ptr)){
		pr_err("%s: no smem allocated\n", __func__);
		return -ENODEV;
	}
	return 0;
}

static void smem_func_delay_cb(struct work_struct *work)
{
	...
	if(1 == run_once){
		value = apps_smem_vendor_init();
		if(value){
			pr_err("%s: apps_smem_vendor_init fail\n",__func__);
			return;
		}
		run_once = 0;
	}
	...
	if(smem_data_ptr){
		pr_err("%s: smem_data_ptr->mdm_cmd = 0x%x\n",__func__,smem_data_ptr->mdm_cmd);
		value = smem_data_ptr->mdm_cmd;
		if (value == 0x5E){
			pr_err("%s: Get efs restore flag from smem\n",__func__);
			mdelay(200);
			do_msm_restart(REBOOT_WARM,"efs_recovery");
		}
	
		if(check_times <= 5){
			check_times++;
			schedule_delayed_work(&smem_func_delay_work,
				msecs_to_jiffies(SMEM_FUNC_DELAY_TIME));
		}else{
			check_times = 0;
		}
	}else{
		pr_err("%s: smem_data_ptr NULL\n",__func__);
	}
	pr_err("%s: Exit\n",__func__);
}
/* maxiaoping  20190927 bugid27493 modification for NV verification,end */
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	/* maxiaoping  20190927 bugid27493 modification for NV verification,start */
	INIT_DELAYED_WORK(&smem_func_delay_work, smem_func_delay_cb);
	schedule_delayed_work(&smem_func_delay_work,
				msecs_to_jiffies(SMEM_FUNC_DELAY_TIME));
	/* maxiaoping  20190927 bugid27493 modification for NV verification,end */
...
static int qpnp_pon_remove(struct spmi_device *spmi)
{
	...
	/* maxiaoping  20190927 bugid27493 modification for NV verification,start */
	cancel_delayed_work_sync(&smem_func_delay_work);
	/* maxiaoping  20190927 bugid27493 modification for NV verification,end */
