...
#include <linux/timekeeping.h>    //yaotong 20170708 changed
...
*/
static int download_mode = 0; //yaotong 20170708 changed
...
/*yaotong 20170708 start*/
	struct timespec ts;
/*yaotong 20170708 end*/
#ifdef CONFIG_MSM_DLOAD_MODE
...
/*yaotong 20170708 start*/
	...
	printk("monotonic time is %d\n", ts.tv_sec);
	if((1 == in_panic) && (ts.tv_sec <= 40))
		{
		   qpnp_pon_system_pwr_off(PON_POWER_OFF_WARM_RESET);
		   if(__raw_readl(restart_reason) == 0x7766550e)
				{
				   printk("panic restart again\n");
...
/*yaotong 20170708 end*/
	...
	if (cmd != NULL) {
		...
		} else if (!strcmp(cmd, "efs_recovery")) {			
			pr_err("%s: come here set efs_recovery\n",__func__);
			qpnp_pon_set_restart_reason(
				PON_RESTART_REASON_RTC);
