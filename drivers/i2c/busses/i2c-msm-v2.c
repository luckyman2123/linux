...
static int i2c_msm_probe(struct platform_device *pdev)
{
	...
	codec_power_gpio = of_get_gpio_flags(pdev->dev.of_node, 0, &flags);
	if (codec_power_gpio < 0) { 
			pr_info("Failed to get gpio flags for codec power, error: %d\n",codec_power_gpio);
	}
    else
    {
        ret = gpio_direction_output(codec_power_gpio, 1);
        if (ret) {
            pr_info("gpio_direction_output for codec power fail %s\n", "codec_power_gpio");
        }
        else
        {
            pr_info("pull up codec_power_success sucess");
        }
    }
	//////////////////////////////////////////////////////////////////
