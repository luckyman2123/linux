...
static int msm_hsl_startup(struct uart_port *port)
{
	...
	if (unlikely(ret)) {
        ...
        if(pdata && pdata->use_pm)                             //[FG][AUTOSLEEP]-yanglonglong-20161129
...
static void msm_hsl_shutdown(struct uart_port *port)
{
	...
	if (!(is_console(port)) || (!port->cons) ||
		...
		if (pdata && pdata->use_pm)                       
			__pm_relax(&msm_hsl_port->port_open_ws);           //[FG][AUTOSLEEP]-yanglonglong-20161129
		  //__pm_stay_awake(&msm_hsl_port->port_open_ws);
	}
}
...
// Begin: Added by lishihong for extending logical device node on 2018-11-09
	{
		.uart = {
			.iotype = UPIO_MEM,
			.ops = &msm_hsl_uart_pops,
...
// End: Added by lishihong for extending logical device node on 2018-11-09
};
