...
/* Bug 15777 wangguanzhi added 20180404 Begin*/
#include <linux/prj_macro_ctrl.h>
/* Bug 15777 wangguanzhi added 20180404 End*/
...
extern void gpio_usb_detect(int vbus_t);
			    ...
			    const char *buff, size_t size)
{
    ...
    //<!-- [BugID 566]caogang@20150718 add auto config reset for sleep func gadget
...
static ssize_t dusb_show(struct device *pdev, struct device_attribute *attr,
			   char *buf)
{
		return snprintf(buf, PAGE_SIZE, "%d\n",dusb_state);		   
}
static ssize_t dusb_store(struct device *pdev, struct device_attribute *attr,
			    const char *buff, size_t size)
{
	int enabled;
	...
	if(dusb_state==0)
	{
		gpio_usb_detect(enabled);
	}
	else if(dusb_state==1)
	{
		gpio_usb_detect(enabled);
	}
	else
...
static DEVICE_ATTR(dusb, S_IRUGO | S_IWUSR, dusb_show,dusb_store);
...
static int android_bind(struct usb_composite_dev *cdev)
{
    ...
    /* Bug 15777 wangguanzhi added 20180404 Begin*/
    if(IS_ENABLED(PRJ_SLM750T_HWL))
    {
        strlcpy(manufacturer_string, "STYX Communication",
           sizeof(manufacturer_string) - 1);
        strlcpy(product_string, "MG-9227GP", sizeof(product_string) - 1);
    }else
    {
    ...
    /* Bug 15777 wangguanzhi added 20180404 End*/
//<!-- [BugID]caogang@20160126 set null serial string number
	//strlcpy(serial_string, "0123456789ABCDEF", sizeof(serial_string) - 1);
	strlcpy(serial_string, "", sizeof(serial_string) - 1);
//end-->
