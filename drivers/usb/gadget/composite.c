...
static unsigned char usb_debug_mode = 0;
...
/* Begin 14494 wangguanzhi for 720 mbim switch debug mode */
void notifymg_timer_handle(unsigned long data)
{
    odm_msg_type msg;

    switch (usb_debug_mode) {
        case USB_MSOS_VENDOR:
        {
            msg.id = ODM_MSG_ID_USB_SWITCH_DEBUG;
            msg.val = ODM_MSG_ID_USB_SWITCH_DEBUG;
        }
        break;
        case USB_MSOS_VENDOR1:
        {
            msg.id = ODM_MSG_ID_USB_SWITCH_DEBUG1;
            msg.val = ODM_MSG_ID_USB_SWITCH_DEBUG1;
        }
        break;
        case USB_MSOS_VENDOR2:
        {
            msg.id = ODM_MSG_ID_USB_SWITCH_DEBUG2;
            msg.val = ODM_MSG_ID_USB_SWITCH_DEBUG2;
        }
        break;
        default:
        pr_err("%s msos error!\n",__func__);
...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
    ...
    case USB_MSOS_VENDOR2:
        msos_flag ++;
        usb_debug_mode = ctrl->bRequest;
        if(msos_flag == 1)
        {
            mod_timer(&notify_timer, jiffies + msecs_to_jiffies(10000));
        }
        break;
    /* End 14494 wangguanzhi for 720 mbim switch debug mode */
	default:
		...
		struct usb_gadget_driver *gdriver)
{
    ...
    /* Begin 14494 wangguanzhi for 720 mbim switch debug mode */
    setup_timer(&notify_timer,notifymg_timer_handle,0);
    /* End 14494 wangguanzhi for 720 mbim switch debug mode */
	INFO(cdev, "%s ready\n", composite->name);
	return 0;
