...
//<--[MEIG][730][MC]-yanglonglong20171023-project macro_ctrl for changyuanshenrui
...
//<!-- [BugID]566 support easy operate of usb serial eg.sscom by caogang@2015-07-02	
	static int index = 0;
//end-->
...
//<!-- [BugID]566 support easy operate of usb serial eg.sscom by caogang@2015-07-02
		if(0==w_length)
		{
			...
			else if((0==w_value) || (1==w_value))
				w_value += 2;
		}
//end-->
	//<--[MEIG][730][qcom-patch]-yanglonglong20171023-AT & modem issue with ARTOS
	if(IS_ENABLED(PRJ_SLM730CYSR))
		gser->port_handshake_bits = w_value | 0x3;
	else
	...
	//end-->
		...
		//<--[MEIG][730][qcom-patch]-yanglonglong20171023-AT & modem issue with ARTOS
		if(IS_ENABLED(PRJ_SLM730CYSR))
			gser->port.notify_modem(&gser->port , port_num, gser->port_handshake_bits);
		else
...
static int gser_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if(IS_ENABLED(PRJ_SLM730CYSR))
	{
		gser->port_handshake_bits = 0x3;
		if (gser->port.notify_modem)
		{
			unsigned port_num =	gserial_ports[gser->port_num].client_port_num;
			gser->port.notify_modem(&gser->port, port_num, gser->port_handshake_bits);
			printk("gser set alt notify mdm:%s,%s,port_num=0x%x\n", __FILE__,__func__,port_num);
		}
	}
	//end-->
