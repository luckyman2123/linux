...
static int acm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		if(IS_ENABLED(PRJ_SLM730CYSR))
			acm->port_handshake_bits = w_value | 0x3;
		else
		...
		//<--[MEIG][730][qcom-patch]-yanglonglong20171023-AT & modem issue with ARTOS
		if(IS_ENABLED(PRJ_SLM730CYSR))
			acm->port.notify_modem(&acm->port, port_num, acm->port_handshake_bits);
		else
