...
//<!--[FG][730][SLEEP]-yanglonglong-20160830
...
static int msm_otg_set_suspend(struct usb_phy *phy, int suspend)
{
	...
	if (suspend) {
		...
		//<!--[FG][730][SLEEP]-yanglonglong-20160830
//	#if defined(CONFIG_USB_SLEEP_FUNC)
		if(!motg->async_int)
			{
				if (atomic_read(&motg->pm_suspended))
                    motg->sm_work_pending = true;
...
//<!--[FG][730][SLEEP]-yanglonglong-20160830
	{
        odm_msg_type msg;
        msg.id = ODM_MSG_ID_USB_SUSPEND;
        msg.val = ODM_MSG_ID_USB_SUSPEND;
	    pr_info("%s:Sending ODM_MSG_ID_USB_SUSPEND message to DM\n",__func__);
...
/*BugID-13526 2+1 branch and 2+2 branch  code combination  weiwenwen 12-05-2017 begin*/
#ifdef CONFIG_USB
	if (motg->host_bus_suspend)
		usb_hcd_resume_root_hub(hcd);

#endif
/*BugID-13526 2+1 branch and 2+2 branch  code combination  weiwenwen 12-05-2017 end*/
	...
	case USB_CHG_STATE_DCD_DONE:
		...
		if (vout && !dm_vlgc) { /* VDAT_REF < DM < VLGC */
			...
			if (line_state) /* DP > VLGC or/and DM > VLGC */
				...
				motg->chg_type = USB_FLOATED_CHARGER;
			/*else if(!dcd)                //yanglonglong-2017-01-22 rollback charge type 
...
//<!--[FG][750][SLEEPEN]-yanglonglong-20170807
/*
 * Add a device attribute for control lpm function about usb
 */
static ssize_t usb_sleepen_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	struct msm_otg *motg = the_msm_otg;
	struct msm_otg_platform_data *pdata = motg->pdata;
...
static ssize_t usb_sleepen_store(struct device *dev,
		struct device_attribute *attr, const char
		*buf, size_t size)
{
	struct msm_otg *motg = the_msm_otg;
	...
	if (!strnicmp(buf, "enable", 6)) {
		pdata->usb_sleepen = true;
		return size;
	} else if (!strnicmp(buf, "disable", 7)) {
		pdata->usb_sleepen = false;
		return size;
	}
...
static DEVICE_ATTR(usleepen, S_IRUGO | S_IWUSR, usb_sleepen_show, usb_sleepen_store);
//end-->
...
static int msm_otg_probe(struct platform_device *pdev)
{
	...
	//<!--[FG][750][SLEEPEN]-yanglonglong-20170807
	device_create_file(&pdev->dev, &dev_attr_usleepen);
	//end-->
...
static int msm_otg_remove(struct platform_device *pdev)
{
	...
	if (motg->pdata->mpm_dpshv_int || motg->pdata->mpm_dmshv_int)
	{
		...
		//<!--[FG][750][SLEEPEN]-yanglonglong-20170807
		device_remove_file(&pdev->dev, &dev_attr_usleepen);
		//end-->
	}
	if (motg->pdata->otg_control == OTG_PHY_CONTROL &&
...
//<!--[FG][730][SLEEP]-yanglonglong-20161201
	{
	        odm_msg_type msg;
        	msg.id = ODM_MSG_ID_USB_RESUME;
       		msg.val = ODM_MSG_ID_USB_RESUME;
	    	pr_info("%s:Sending ODM_MSG_ID_USB_RESUME message to DM\n",__func__);
       		odm_notify(&msg, sizeof(msg));
	}
//end-->
