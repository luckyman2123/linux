...
#include <linux/udp.h>
#include <linux/in.h>
// #include <linux/prj_macro_ctrl_IAD.h>

#define RTMGRP_ETH_LINK_PORT 1 
#define NETLINK_ETH_GRP 25
struct sock *sock_t;
...
static void (*qca_rcv_cb)(struct net_device *dev, u16 type, void *msg, int len);
...
void send_netlink_data(void *message)  
{      
	...
	if(0 == eth_event_socket.ref_cnt)
	{
		printk("qca8337 socket event init error, ref_cnt is %d\n", eth_event_socket.ref_cnt);
	}
	...
	nlh = nlmsg_put(skb_out, 0, 1, 0, msg_size, NLM_F_MULTI);
	if(!nlh)
	{
		printk("Error while nlmsg_put skb_out\n");   
	}
	
	memcpy(nlmsg_data(nlh), message, msg_size);
	res = netlink_broadcast(eth_event_socket.sock, skb_out, 0, RTMGRP_ETH_LINK_PORT, GFP_KERNEL);
	if (res < 0) 
	{
		printk("Error while sending bak to user, err id: %d\n", res);   
	}
}  
...
{    
#if 0
	struct sk_buff *skb;      
	struct nlmsghdr *nlh;      //struct completion cmpl;      
	int i=10;    
	char str[1024 *2]; 
	int pid;
	int flag = 0;
	...
	{          
		...
		while(i--)          
		{              
			send_netlink_data("kernel recv msg ok!\n");          
		}          
		flag = 1;          
		kfree_skb(skb);      
	}  
...
struct sock *qca8337_netlink_init(int unit)
{
	struct netlink_kernel_cfg cfg = {
		.input	= recv_netlink_data,
		.groups  = RTMGRP_ETH_LINK_PORT,    
	};

	return netlink_kernel_create(&init_net, unit, &cfg);
}

static int qca8337_socket_event_init(void)
{
	eth_event_socket.sock = qca8337_netlink_init(NETLINK_ETH_GRP);

	if (!eth_event_socket.sock) 
	{
		pr_err("Creating qca8337 Event netlink is failed\n");
		return -1;
	}
...
static void qca8337_socket_event_exit(void)
{
	if (eth_event_socket.sock && --eth_event_socket.ref_cnt == 0) 
	{
		netlink_kernel_release(eth_event_socket.sock);
		eth_event_socket.sock = NULL;
	}
}
...
void qca8337_vlan_config(struct qca8337_priv *priv)
{
#if 0
...
#endif
	priv->ops->write(priv, QCA8337_REG_PORT_LOOKUP(0), 0x0014027e);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN0(0), 0x00020001);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN1(0), 0x00001040);

	priv->ops->write(priv, QCA8337_REG_PORT_LOOKUP(1), 0x0014027d);
#ifdef ENABLE_IAD_SERVER
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN0(1), 0x00030001);
#else
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN0(1), 0x00020001);
#endif
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN1(1), 0x00001140);

	priv->ops->write(priv, QCA8337_REG_PORT_LOOKUP(2), 0x0014027b);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN0(2), 0x00020001);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN1(2), 0x00001140);

	priv->ops->write(priv, QCA8337_REG_PORT_LOOKUP(3), 0x00140277);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN0(3), 0x00020001);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN1(3), 0x00001140);

	priv->ops->write(priv, QCA8337_REG_PORT_LOOKUP(4), 0x0014026f);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN0(4), 0x00020001);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN1(4), 0x00001140);
 	
	priv->ops->write(priv, QCA8337_REG_VTU_FUNC0, 0x001bd560);
	priv->ops->write(priv, QCA8337_REG_VTU_FUNC1, 0x80020002);
}

void qca8337_vlan_config_ex(struct qca8337_priv *priv)
{
	priv->ops->write(priv, QCA8337_REG_PORT_LOOKUP(0), 0x0014027e);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN0(0), 0x00010001);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN1(0), 0x00002040);
	...
	priv->ops->write(priv, QCA8337_REG_PORT_LOOKUP(4), 0x0014026f);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN0(4), 0x00010001);
	priv->ops->write(priv, QCA8337_REG_PORT_VLAN1(4), 0x00001140);
	
	priv->ops->write(priv, QCA8337_REG_VTU_FUNC0, 0x001bdfe0);
	priv->ops->write(priv, QCA8337_REG_VTU_FUNC1, 0x80010002);
#ifdef ENABLE_IAD_SERVER	
	//priv->ops->write(priv, QCA8337_REG_VTU_FUNC0, 0x001bf560);
	priv->ops->write(priv, QCA8337_REG_VTU_FUNC0, 0x001bf5e0);
	priv->ops->write(priv, QCA8337_REG_VTU_FUNC1, 0x80020002);
    /*add vlan3*/
	priv->ops->write(priv, QCA8337_REG_VTU_FUNC0, 0x001bff60); 
	priv->ops->write(priv, QCA8337_REG_VTU_FUNC1, 0x80030002);
#else
	priv->ops->write(priv, QCA8337_REG_VTU_FUNC0, 0x001bf560);
	priv->ops->write(priv, QCA8337_REG_VTU_FUNC1, 0x80020002);
#endif
...
static int qca8337_hw_init(struct qca8337_priv *priv)
{
	...
	qca8337_vlan_config_ex(priv);		//chenquan
...
static void qca8337_phy_enable(struct phy_device *phydev)
{
	...
	if (AUTONEG_ENABLE == phydev->autoneg) {
		...
		for (port = 1; port < priv->ports - 1; port++) {
			...
			status |= (advertise & ADVERTISED_Asym_Pause) ?
				QCA8337_PORT_STATUS_TXFLOW : 0;
			status |= (advertise & ADVERTISED_Pause) ?
...
int qca8337_config_aneg(struct phy_device *phydev)
{
...
static int qca8337_read_status(struct phy_device *phydev)
{
	...
	/*bugid:12659, modified by yaoguangdong, 2017-12-01, start*/
	if(port_status != g_port_status)
	{
		g_port_status = port_status;
		send_netlink_data((void *)&g_port_status);
	}
	/*bugid:12659, modified by yaoguangdong, 2017-12-01, end*/
	return ret;
}
...
static struct regmap_access_table qca8337_readable_table = {
...
{
	return phy_driver_register(&qca8337_driver);
}
