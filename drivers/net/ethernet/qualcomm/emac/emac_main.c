...
#define AR8033_PHY_ID 0x004dd074 // Added by lishihong for BugID 15739 on 2018-04-01
...
static int emac_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
	struct emac_adapter *adpt = netdev_priv(netdev);
	struct emac_hw *hw = &adpt->hw;
	struct phy_device *pdev = netdev->phydev;
	...
	switch (cmd) {
	case CTRLETHONLYLANPORTENABLE:
		qca8337_vlan_config(pdev->priv);
		return 0;
	case CTRLETHWANPORTENABLE:
		qca8337_vlan_config_ex(pdev->priv);
		return 0;
	/*bugid:12659, modified by yaoguangdong, 2017-12-01, start*/
	case CTRLGETETHSTATUS:
		ifr->ifr_ifru.ifru_ivalue = g_port_status;
		return 0;
	/*bugid:12659, modified by yaoguangdong, 2017-12-01, end*/
    case CTRLGETETHCHIP:
        ifr->ifr_ifru.ifru_ivalue = pdev->phy_id;
        return 0;
	case SIOCSHWTSTAMP:
		if (TEST_FLAG(hw, HW_PTP_CAP))
...
static int emac_pm_suspend(struct device *device, bool wol_enable)
{
	...
	// Begin: Added by lishihong for BugID 15739 on 2018-04-01
	if (AR8033_PHY_ID == adpt->phydev->phy_id)
	{
		pm_runtime_get(device);
	}
	// End: Added by lishihong for BugID 15739 on 2018-04-01
...
err_clk_init:
	//if (ATH8030_PHY_ID == adpt->phydev->phy_id)  // Under the switch hardware fault scenario, CPE cannot start up properly
		emac_disable_clks(adpt);
err_ldo_init:
	//if (ATH8030_PHY_ID == adpt->phydev->phy_id)  // Modfied by lishihong on 2017-08-02
