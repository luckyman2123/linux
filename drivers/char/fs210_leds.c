/***********************************************************************************
* drivers/char/fs210_leds.c
* 功能简要： 
*	该驱动注册一个字符设备“/dev/led”, 用于2个LED。
* 函数简介：
*	static void fs210_debug_leds(unsigned int cmd,unsigned long arg),用于内核驱动调试
* 提供的外部接口：
...
#ifdef CONFIG_FS210_DEBUG_LEDS
static void fs210_debug_leds(unsigned int cmd,unsigned long arg)
{
...
**/
static int fs210_gpio_open(struct inode *inode, struct file *file)
{
	int i;
	int err;
...
static int fs210_gpio_close(struct inode *inode, struct file *file)
{
	gpio_free(gpio_table[0]);
	gpio_free(gpio_table[1]);
	printk(KERN_INFO "FS210 LEDs driver successfully close\n");
	return 0;
}
...
static struct file_operations dev_fops = {
	.owner	=	THIS_MODULE,
	.unlocked_ioctl	=	fs210_gpio_ioctl,
	.open = fs210_gpio_open,
	.release = fs210_gpio_close,
};
/*设备结构的设置*/
...
static int __init dev_init(void)
{
	...
	printk(KERN_INFO "FS210 LEDs driver successfully probed\n");
...
static void __exit dev_exit(void)
{
	...
	gpio_free(gpio_table[0]);
	gpio_free(gpio_table[1]);
	printk(KERN_INFO "FS210 LEDs driver successfully exit\n");
}
